  ...
  useEffect(() => {
    const obtenerDetalleDenuncia = async () => {
      try {
        ...
        if (data.puntoGPS) {
          const [latitude, longitude] = data.puntoGPS.split(",").map(Number);
          setInitialRegion({
            latitude,
            longitude,
            latitudeDelta: 0.005,
        ...
        console.error("Error al obtener el detalle de la denuncia:", error);
        Alert.alert("Error", "No se pudo obtener el detalle de la denuncia.");
      }
    };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Card style={styles.card}>
          ...
          <Card.Content>
            ...
            <Text>{denuncia.direccion || "Dirección no especificada"}</Text>
          </Card.Content>
        </Card>
          ...
          {initialRegion && (
            <MapView style={styles.map} region={initialRegion}>
              <Marker
                ...
                description={denuncia.direccion || "Ubicación aproximada"}
              />
            </MapView>
          )}
        </View>
        ...
        <View style={styles.seccionContainer}>
          <Text style={styles.seccionTitulo}>Eventos Relacionados</Text>
          {denuncia.SolicitudEventoPersonas.length > 0 ? (
            denuncia.SolicitudEventoPersonas.map((evento, index) => (
              <Card key={index} style={styles.eventoCard}>
                <Card.Content>
                  <Text style={styles.eventoText}>{evento.id_evento}</Text>
                  <Text style={styles.eventoAutor}>
                    Modificado por: {evento.persona.nombres}{" "}
              ...
              <Card key={index} style={styles.observacionCard}>
                <Card.Content>
                  ...
                  <Text style={styles.observacionAutor}>
                    Modificado por: {observacion.persona.nombres}{" "}
            ...
            <View style={styles.estadoContainer}>
              ...
              >
                <Text
                  style={[
                    styles.estadoText,
                    estadoCierre === "Resuelto" && { color: "#155724" },
                  ]}
                >
                  Resuelto
                </Text>
              ...
              >
                <Text
                  style={[
                    styles.estadoText,
                    estadoCierre === "Falso" && { color: "#721c24" },
                  ]}
                >
                  Falso
                </Text>
